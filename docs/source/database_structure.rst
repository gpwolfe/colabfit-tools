...
stored in a PostgreSQL database with five core data structures:

* :class:`~colabfit.tools.pg.configuration.AtomicConfiguration` (CO, :code:`table='configurations'`):
...
* :class:`~colabfit.tools.pg.property.Property` (PI, :code:`table='property_objects'`):
      ...
      one or more individual :class:`AtomicConfiguration`
      instances; if the :class:`AtomicConfiguration` objects that a property points to
...
* :class:`Property Definition` (PD, :code:`table='property_definitions'`):
...
* :class:`~colabfit.tools.pg.configuration_set.ConfigurationSet` (CS, :code:`table='configuration_sets'`):
      An object defining a group of one or more :class:`AtomicConfiguration` instances
      and providing useful metadata for organizing datasets (e.g., "Snapshots
      from a molecular dynamics run at 1000K").
* :class:`~colabfit.tools.pg.dataset.Dataset` (DS, :code:`table='datasets'`):
